...
if(isset($_POST['creditItemId'])){
    $creditItemId = $_POST['creditItemId'];

    $getcreditItemInformation = "SELECT *FROM credit_item WHERE id = '$creditItemId'";
    $creditItemInfo = $db->link->query($getcreditItemInformation);

    echo json_encode($creditItemInfo->fetch_array());
}
...
if(isset($_POST['itemNameId'])){
    ...
    $updateDebitItemName = "UPDATE credit_item SET item_name = '$itemName'  WHERE `credit_item`.`id` = '$itemNameId'";
