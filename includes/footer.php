		...
		<script type="text/javascript"
		    src="https://cdn.datatables.net/v/dt/jszip-2.5.0/dt-1.10.18/b-1.5.4/b-colvis-1.5.4/b-flash-1.5.4/b-html5-1.5.4/b-print-1.5.4/cr-1.5.0/fc-3.2.5/r-2.2.2/rr-1.2.4/sc-1.5.0/sl-1.2.6/datatables.min.js">
		</script>
		...
		<script>
		    ...
		    $('.status_btn').click(function () {
		        ...
		        $.ajax({
		            ...
		            success: function (data) {
		                if (data == '0') {
		                    $("#" + userId).closest('span.btn_green').removeClass('btn_green').addClass(
		                        'btn_red');
		                } else if (data == 1) {
...
<?php
if($uri_parts[0] == '/staff_list_old.php'){
    ?>
		...
		<script>
		    ...
		    $("#add_product").click(function () {
		        ...
		        if (check != 1) {
		            ...
		            $("#item_list").append(
		                '<tr> <td class="ser" style = "text-align:center;" ><input class="remove_p" type="hidden" name="product_id[]" value="' +
		                product_id + '" /><span>' + ser + '</span> </td> <td style = "text-align:center;" > ' +
		                productName +
		                ' </td> <td style = "text-align:center;" ><input type="number" min="1" max="' +
		                parseInt(sale_quantit) + '" name="quantity[]" value="' + quantity +
		                '" onkeyup="update_total(event)" onclick="update_total(event)" class="up_quantity"></td> <td style = "text-align:right;" ><input onkeyup="update_total(event)" class="up_price" type="text" name="sale_price[]" value=' +
		                salePrice + ' min="' + buy_price +
...
?>
		<script>
		    ...
		    function quantity(event) {
		        ...
		        $(event.target).closest('tr').find('.quntity_total').append(
		            '<input type="hidden" name="quntityTotal[]" value="' +
		            total + '">');
		    ...
		    function remove_product(event) {
		        ...
		        $(event.target).closest('tr').find('.quntity_total').append(
		            '<input type="hidden" name="quntityTotal[]" value="0.00">');
...
?>
<?php
	if($uri_parts[0] == '/agent_update.php'){
?>
		<script>
		$(document).ready(function(){
			$("#agentTable").DataTable({
		...
		$(".editAgentBtn").click(function(){
			...
			// $("#agentIdShow").text(agentId);

			$.ajax({
				url:"lib/ajax_agent_update.php",
				method:"POST",
				data:{
					agentId:agentId
				},
				...
				success:function(agentDetails){
					//set to modal
					$("#agentId").val(agentDetails.id);
					$("#agentName").val(agentDetails.name);
					$("#agentIdShow").text(agentDetails.name);
					$("#agentEmail").val(agentDetails.email);
					$("#agentAssignTo").val(agentDetails.assignto);
					$("#agentAssignTo").selectpicker('refresh');
					$("#contact1").val(agentDetails.contact1);
					$("#contact2").val(agentDetails.contact2);
					$("#address").val(agentDetails.address);
				}
			})
		});

		$("#updateAgentInformation").submit(function(e){
			// $(".loading-img").css("display","block");
			e.preventDefault();
			var agentUpdateInfo = $("#updateAgentInformation").serialize();
			var con = confirm("Are you confirm?");			
			if(con == 1){				
				$.ajax({
				url:"lib/ajax_agent_update.php",
				method: "POST",
				data: agentUpdateInfo,
				success:function(result){
					if(result == 1){						
					location.reload();
					}
    ...
    ?>
		...
		<script>
			...
			$(".editStaffPassword").click(function(){
				var userIdUpdatePass = $(this).prop("id");


				$.ajax({
					url:"lib/ajax_change_pass_all.php",
					method:"POST",
					data:{
						userIdUpdatePassGetName:userIdUpdatePass
					},
					success:function(result){						
					$("#userIdUpdatePass").text(result);
					}
				})

				$("#userIdUpdate").val(userIdUpdatePass);

				// console.log(userIdUpdatePass);
			})
			var st = 0;
			function checkPass(){				
				var confirmPass_all = $("#confirmPass_all").val();
				var newPass_all = $("#newPass_all").val();

				if(st != 0){
					if(confirmPass_all != newPass_all){					
					$("#submit_button").attr("disabled", true);

					$("#newPass_all").closest("div").addClass("has-error");
					$("#newPass_all").closest("div").removeClass("has-success");
					$("#checkNewPassConfirm").html("*Both are not same");
					$("#checkMarkConfirm").css("display", "none");
					$("#closeMarkConfirm").css("display", "block");
				}else{	
					$("#submit_button").attr("disabled", false);				

					$("#newPass_all").closest("div").addClass("has-success");
					$("#newPass_all").closest("div").removeClass("has-error");

					
					$("#confirmPass_all").closest("div").addClass("has-success");
					$("#confirmPass_all").closest("div").removeClass("has-error");

					$("#checkMarkConfirm").css("display", "block");
					$("#closeMarkConfirm").css("display", "none");
					$("#checkNewPassConfirm").html("");
				}
				}else{
					$("#submit_button").attr("disabled", true);
					$("#newPass_all").closest("div").addClass("has-error");
					$("#newPass_all").closest("div").removeClass("has-success");
					$("#checkNewPassConfirm").html("*Both are not same");					
					$("#checkNewPassConfirm").css("color","red");
					$("#checkMarkConfirm").css("display", "none");
					$("#closeMarkConfirm").css("display", "block");					
				}
		    ...
		    $("#newPass_all").keyup(function () {		

		        var newPass_all = $(this).val();
				if(newPass_all != ""){
					...
					success:function(checkResult){
						...
						if (checkResult == 1) {
								...
								$("#confirmPass_all").closest("div").addClass("has-error");
								$("#confirmPass_all").closest("div").removeClass("has-success");
								$("#checkMarkConfirm").css("display", "none");
								$("#closeMarkConfirm").css("display", "block");
								st = 0;
		                        //		                    alert("Password at least 8 Characters");
					...
					$("#newPass_all").closest("div").addClass("has-error");
		            $("#checkNewPass").html("*Password Field Empty!");
					$("#checkMarkNew").css("display", "none");
					$("#submit_button").attr("disabled", true);
			...
			$("#confirmPass_all").keyup(function(){
				
				var confirmPass_all = $(this).val();
				var newPass_all = $("#newPass_all").val();
			...
			$("#updateStaffPassword_all").submit(function(e){
				e.preventDefault();
				var all_pass = $("#updateStaffPassword_all").serialize();
...
<?php
if($uri_parts[0] == '/add_debit_item.php'){
	?>
	<script>
		$(".editDebitItemBtn").click(function(){
			var itemId = $(this).attr("id");

			//code for getting information from debit item table
			$.ajax({
				url:"lib/ajax_debit_item.php",
				method: "POST",
				data:{
					debitItemId : itemId
				},
		...
		$("#updateItemName").submit(function(event){
			event.preventDefault();

			var itemName = $("#updateItemName").serialize();
...
<?php
if($uri_parts[0] == "/add_credit_item.php"){
	?>
	<script>
		$(".editcreditItemBtn").click(function(){
			var itemId = $(this).attr("id");
			
			//code for getting information from debit item table
			$.ajax({
				url:"lib/ajax_credit_item.php",
				method: "POST",
				data:{
					creditItemId : itemId
				},
